...
//    }
    ...
    if (primaryKeyName != "")
    {
        ...
        const char * tmp = indexName.c_str();
...
void API::recordInsert(string tableName, vector<string>* recordContent)
{
    ...
    vector<Attribute> attributeTest;
    attributeGet(tableName, &attributeTest);
    for(int i=0;i<attributeTest.size();i++)printf("atTestNew: %s\n",attributeTest[i].index.c_str());
    attributeGet(tableName, &attributeVector);
    cm->attributeGet(tableName,&attributeTest);
    for(int i=0;i<attributeTest.size();i++)printf("atTest: %s\n",attributeTest[i].index.c_str());
    for (int i = 0 ; i < attributeVector.size(); i++)
    {
        string indexName = attributeVector[i].index;
        cout<<"indexName: "<<attributeVector[i].index<<endl;
    ...
    cm->recordStringGet(tableName, recordContent, recordString, &attributeVector);
    ...
    if (conditionVector != NULL)
    {
        for (Condition condition : *conditionVector)
        {
            if (condition.operate == Condition::OPERATOR_EQUAL)
            {
                for (Attribute attribute : attributeVector)
                {
                    if (attribute.index.compare("") && attribute.name == condition.attributeName)
                    {
...
void API::recordIndexInsert(char* recordBegin,int recordSize, vector<Attribute>* attributeVector,  int blockOffset)
{
    ...
    for (int i = 0; i < (*attributeVector).size() ; i++)
    {
        ...
        if ((*attributeVector)[i].index.compare(""))
        {
